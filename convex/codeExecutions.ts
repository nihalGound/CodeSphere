...
export const saveExecution = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("codeExcecutions", {
      ...args,
...
export const getUserStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const snippetIdx = starredSnippets.map((star) => star.snippetId);
    const snippetDetails = await Promise.all(
      snippetIdx.map((id) => ctx.db.get(id))
    );
    ...
    const favoriteLanguage = languageStats.length
